-- Blank Space v116 update
-- Run this in Supabase before uploading Blank Space v116.zip.
-- Adds True Members, banners, online presence, friend request settings, blocks, game presence, Timed Message fix, and a rebuilt achievement backend.
...
create table if not exists public.admin_user_messages (
  ...
  label text default 'Blank Space Message',
...
alter table public.admin_user_messages add column if not exists sender_id uuid references public.profiles(id) on delete set null;
alter table public.admin_user_messages add column if not exists target_user_id uuid references public.profiles(id) on delete cascade;
alter table public.admin_user_messages add column if not exists title text default 'Blank Space Notice';
alter table public.admin_user_messages add column if not exists label text default 'Blank Space Message';
alter table public.admin_user_messages add column if not exists message text;
alter table public.admin_user_messages add column if not exists style text default 'notice';
alter table public.admin_user_messages add column if not exists active boolean default true;
alter table public.admin_user_messages add column if not exists created_at timestamptz default now();
alter table public.admin_user_messages add column if not exists expires_at timestamptz default now();
...
create or replace function public.is_blank_space_owner_or_admin_v115()
returns boolean language sql stable security definer set search_path=public as $$
  select exists(select 1 from public.profiles p where p.id=auth.uid() and (lower(coalesce(p.rank,'member')) in ('owner','admin') or lower(coalesce(p.username,'')) in ('owner','admin')));
$$;
grant execute on function public.is_blank_space_owner_or_admin_v115() to anon, authenticated;

create or replace function public.admin_set_true_member_v115(p_username text, p_true_member boolean default true)
returns jsonb language plpgsql security definer set search_path=public as $$
declare v_target public.profiles%rowtype;
begin
  if auth.uid() is null or not public.is_blank_space_owner_or_admin_v115() then raise exception 'Owner/Admin only.'; end if;
  ...
  update public.profiles set true_member=coalesce(p_true_member,true) where id=v_target.id;
  insert into public.admin_actions_log(admin_id,target_user_id,target_username,action,reason) values(auth.uid(),v_target.id,v_target.username,case when p_true_member then 'make_true_member' else 'remove_true_member' end,null);
  return jsonb_build_object('ok',true,'username',v_target.username,'true_member',p_true_member);
end $$;
grant execute on function public.admin_set_true_member_v115(text, boolean) to authenticated;

create or replace function public.set_friend_requests_enabled_v115(p_enabled boolean)
returns jsonb language plpgsql security definer set search_path=public as $$
begin
  if auth.uid() is null then raise exception 'Sign in first.'; end if;
  update public.profiles set friend_requests_enabled=coalesce(p_enabled,true) where id=auth.uid();
  return jsonb_build_object('ok',true,'enabled',coalesce(p_enabled,true));
end $$;
grant execute on function public.set_friend_requests_enabled_v115(boolean) to authenticated;

create or replace function public.block_user_by_username_v115(p_username text)
returns jsonb language plpgsql security definer set search_path=public as $$
...
begin
  ...
  if v_target=auth.uid() then raise exception 'You cannot block yourself.'; end if;
  insert into public.friend_blocks(blocker_id,blocked_id) values(auth.uid(),v_target) on conflict do nothing;
  delete from public.friends where (requester_id=auth.uid() and receiver_id=v_target) or (requester_id=v_target and receiver_id=auth.uid());
  return jsonb_build_object('ok',true,'username',v_name);
end $$;
grant execute on function public.block_user_by_username_v115(text) to authenticated;

create or replace function public.unblock_user_by_username_v115(p_username text)
returns jsonb language plpgsql security definer set search_path=public as $$
declare v_target uuid; v_name text;
begin
  if auth.uid() is null then raise exception 'Sign in first.'; end if;
  select id,username into v_target,v_name from public.profiles where lower(trim(username))=lower(trim(coalesce(p_username,''))) limit 1;
  if v_target is null then raise exception 'User not found.'; end if;
  delete from public.friend_blocks where blocker_id=auth.uid() and blocked_id=v_target;
  return jsonb_build_object('ok',true,'username',v_name);
end $$;
grant execute on function public.unblock_user_by_username_v115(text) to authenticated;

create or replace function public.send_friend_request_v115(p_username text)
returns jsonb language plpgsql security definer set search_path=public as $$
declare v_target public.profiles%rowtype; v_existing bigint;
begin
  if auth.uid() is null then raise exception 'Sign in first.'; end if;
  select * into v_target from public.profiles where lower(trim(username))=lower(trim(coalesce(p_username,''))) limit 1;
  if v_target.id is null then raise exception 'No account found with that username.'; end if;
  if v_target.id=auth.uid() then raise exception 'You cannot add yourself.'; end if;
  if coalesce(v_target.friend_requests_enabled,true)=false then raise exception 'User has friend requests turned off.'; end if;
  if exists(select 1 from public.friend_blocks where (blocker_id=auth.uid() and blocked_id=v_target.id) or (blocker_id=v_target.id and blocked_id=auth.uid())) then raise exception 'Friend request blocked.'; end if;
  select id into v_existing from public.friends where (requester_id=auth.uid() and receiver_id=v_target.id) or (requester_id=v_target.id and receiver_id=auth.uid()) limit 1;
  if v_existing is not null then raise exception 'A friend request already exists.'; end if;
  insert into public.friends(requester_id,receiver_id,status) values(auth.uid(),v_target.id,'pending');
  return jsonb_build_object('ok',true,'username',v_target.username);
end $$;
grant execute on function public.send_friend_request_v115(text) to authenticated;

create or replace function public.buy_banner_pack_v115(p_pack_id text,p_price integer,p_banner_id text,p_name text,p_rarity text,p_gradient text)
returns jsonb language plpgsql security definer set search_path=public as $$
declare v_points int;
begin
  if auth.uid() is null then raise exception 'Sign in first.'; end if;
  select points into v_points from public.profiles where id=auth.uid() for update;
  if coalesce(v_points,0)<coalesce(p_price,0) then raise exception 'Not enough points.'; end if;
  perform set_config('blank_space.allow_points_update','1',true);
  update public.profiles set points=coalesce(points,0)-greatest(0,coalesce(p_price,0)) where id=auth.uid();
  insert into public.user_banners(user_id,banner_id,name,rarity,gradient) values(auth.uid(),p_banner_id,p_name,p_rarity,p_gradient) on conflict(user_id,banner_id) do update set acquired_at=now();
...
create or replace function public.equip_banner_v115(p_banner_id text)
returns jsonb language plpgsql security definer set search_path=public as $$
declare b public.user_banners%rowtype;
begin
  if auth.uid() is null then raise exception 'Sign in first.'; end if;
  select * into b from public.user_banners where user_id=auth.uid() and banner_id=p_banner_id limit 1;
  if b.user_id is null then raise exception 'You do not own this banner.'; end if;
  update public.profiles set equipped_banner_id=b.banner_id,equipped_banner_name=b.name,equipped_banner_gradient=b.gradient where id=auth.uid();
  return jsonb_build_object('ok',true,'banner_id',b.banner_id);
end $$;
grant execute on function public.equip_banner_v115(text) to authenticated;

create or replace function public.admin_send_user_message_v115(p_username text,p_message text,p_seconds integer default 20,p_style text default 'notice',p_send_all boolean default false)
returns jsonb language plpgsql security definer set search_path=public as $$
declare v_target public.profiles%rowtype; v_count int:=0; v_seconds int:=greatest(5,least(coalesce(p_seconds,20),3600));
begin
  if auth.uid() is null or not public.is_blank_space_owner_or_admin_v115() then raise exception 'Owner/Admin only.'; end if;
  if trim(coalesce(p_message,''))='' then raise exception 'Message is required.'; end if;
  if coalesce(p_send_all,false) then
    insert into public.admin_user_messages(target_user_id,sender_id,title,label,message,style,active,expires_at)
    select id,auth.uid(),'Blank Space Notice','Blank Space Message',left(p_message,500),coalesce(p_style,'notice'),true,now()+(v_seconds||' seconds')::interval from public.profiles where id<>auth.uid();
    get diagnostics v_count = row_count;
  else
    select * into v_target from public.profiles where lower(trim(username))=lower(trim(coalesce(p_username,''))) limit 1;
    if v_target.id is null then raise exception 'User not found.'; end if;
    insert into public.admin_user_messages(target_user_id,sender_id,title,label,message,style,active,expires_at) values(v_target.id,auth.uid(),'Blank Space Notice','Blank Space Message',left(p_message,500),coalesce(p_style,'notice'),true,now()+(v_seconds||' seconds')::interval);
    v_count:=1;
  end if;
  insert into public.admin_actions_log(admin_id,target_username,action,reason) values(auth.uid(),case when p_send_all then 'everyone_except_sender' else p_username end,'timed_message',left(p_message,300));
  return jsonb_build_object('ok',true,'sent',v_count);
end $$;
grant execute on function public.admin_send_user_message_v115(text,text,integer,text,boolean) to authenticated;

-- Achievement catalog and awarding
create table if not exists public.game_stats (user_id uuid not null references public.profiles(id) on delete cascade, game_title text not null, play_count integer default 0, play_seconds integer default 0, first_played_at timestamptz default now(), last_played_at timestamptz default now(), primary key(user_id,game_title));
alter table public.game_stats add column if not exists best_score integer default 0; alter table public.game_stats add column if not exists goals integer default 0; alter table public.game_stats add column if not exists wins integer default 0; alter table public.game_stats add column if not exists fish_caught integer default 0; alter table public.game_stats add column if not exists money_earned integer default 0; alter table public.game_stats add column if not exists checks integer default 0; alter table public.game_stats add column if not exists checkmates integer default 0; alter table public.game_stats add column if not exists merges integer default 0; alter table public.game_stats add column if not exists best_level integer default 0; alter table public.game_stats add column if not exists hits integer default 0; alter table public.game_stats add column if not exists snake_mode text;
create table if not exists public.user_achievements (user_id uuid not null references public.profiles(id) on delete cascade, achievement_id text not null, title text default 'Achievement', description text default '', icon text default '🏆', game_title text, reward_points integer default 0, earned_at timestamptz default now(), primary key(user_id,achievement_id));
create table if not exists public.achievement_catalog (achievement_id text primary key, title text, description text, icon text, game_title text, stat text, target integer, reward_points integer, difficulty text);
insert into public.achievement_catalog(achievement_id,title,description,icon,game_title,stat,target,reward_points,difficulty) values
  ...
  ('slope_best_score_25_18','Get 5 points in Slope','Get 5 points in one Slope round','🟢','Slope','best_score',5,2,'not_so_hard'),
  ('slope_best_score_50_19','Get 10 points in Slope','Get 10 points in one Slope round','🟢','Slope','best_score',10,2,'not_so_hard'),
  ('slope_best_score_75_20','Get 15 points in Slope','Get 15 points in one Slope round','🟢','Slope','best_score',15,2,'not_so_hard'),
  ('slope_best_score_100_21','Get 20 points in Slope','Get 20 points in one Slope round','🟢','Slope','best_score',20,2,'not_so_hard'),
  ('slope_best_score_125_22','Get 30 points in Slope','Get 30 points in one Slope round','🟢','Slope','best_score',30,2,'not_so_hard'),
  ('slope_best_score_150_23','Get 40 points in Slope','Get 40 points in one Slope round','🟢','Slope','best_score',40,2,'not_so_hard'),
  ('slope_best_score_200_24','Get 50 points in Slope','Get 50 points in one Slope round','🟢','Slope','best_score',50,5,'medium'),
  ('slope_best_score_250_25','Get 60 points in Slope','Get 60 points in one Slope round','🟢','Slope','best_score',60,5,'medium'),
  ('slope_best_score_300_26','Get 75 points in Slope','Get 75 points in one Slope round','🟢','Slope','best_score',75,5,'medium'),
  ('slope_best_score_400_27','Get 90 points in Slope','Get 90 points in one Slope round','🟢','Slope','best_score',90,5,'medium'),
  ('slope_best_score_500_28','Get 110 points in Slope','Get 110 points in one Slope round','🟢','Slope','best_score',110,5,'medium'),
  ('slope_best_score_650_29','Get 130 points in Slope','Get 130 points in one Slope round','🟢','Slope','best_score',130,10,'hard'),
  ('slope_best_score_800_30','Get 150 points in Slope','Get 150 points in one Slope round','🟢','Slope','best_score',150,10,'hard'),
  ('slope_best_score_1000_31','Get 175 points in Slope','Get 175 points in one Slope round','🟢','Slope','best_score',175,10,'hard'),
  ('slope_best_score_1250_32','Get 200 points in Slope','Get 200 points in one Slope round','🟢','Slope','best_score',200,10,'hard'),
  ...
  ('slope_best_score_1100_201','Get 250 points in Slope','Get 250 points in one Slope round','🟢','Slope','best_score',250,10,'hard'),
  ...
  ('slope_best_score_1150_211','Get 250 points in Slope','Get 250 points in one Slope round','🟢','Slope','best_score',250,20,'crazy'),
  ...
  ('slope_best_score_1200_221','Get 250 points in Slope','Get 250 points in one Slope round','🟢','Slope','best_score',250,20,'crazy'),
  ...
  ('slope_best_score_1250_231','Get 250 points in Slope','Get 250 points in one Slope round','🟢','Slope','best_score',250,20,'crazy'),
  ...
  ('slope_best_score_1300_241','Get 250 points in Slope','Get 250 points in one Slope round','🟢','Slope','best_score',250,20,'crazy'),
...
create table if not exists public.aswc_goal_signal_pairs(user_id uuid primary key references public.profiles(id) on delete cascade,pending_signals integer not null default 0,updated_at timestamptz not null default now());

create or replace function public.award_ready_achievements(p_user_id uuid,p_game_title text default null)
returns integer language plpgsql security definer set search_path=public as $$
declare v_auth uuid:=auth.uid(); v_awards int:=0; v_points int:=0; v_game text:=p_game_title;
begin
  if v_auth is not null and p_user_id is distinct from v_auth then raise exception 'Not allowed to award another user.'; end if;
  if lower(trim(coalesce(v_game,''))) in ('slope','slope game') then v_game:='Slope'; end if;
  if lower(trim(coalesce(v_game,''))) in ('a small world cup','a-small-world-cup','small world cup') then v_game:='A Small World Cup'; end if;
  with eligible as (select c.* from public.achievement_catalog c join public.game_stats gs on gs.user_id=p_user_id and gs.game_title=c.game_title left join public.user_achievements ua on ua.user_id=p_user_id and ua.achievement_id=c.achievement_id where ua.achievement_id is null and (v_game is null or c.game_title=v_game) and (case c.stat when 'play_count' then coalesce(gs.play_count,0) when 'best_score' then coalesce(gs.best_score,0) when 'goals' then coalesce(gs.goals,0) when 'wins' then coalesce(gs.wins,0) when 'fish_caught' then coalesce(gs.fish_caught,0) when 'money_earned' then coalesce(gs.money_earned,0) when 'checks' then coalesce(gs.checks,0) when 'checkmates' then coalesce(gs.checkmates,0) when 'merges' then coalesce(gs.merges,0) when 'best_level' then coalesce(gs.best_level,0) when 'hits' then coalesce(gs.hits,0) else 0 end)>=coalesce(c.target,999999999)), inserted as (insert into public.user_achievements(user_id,achievement_id,title,description,icon,game_title,reward_points,earned_at) select p_user_id,achievement_id,title,description,icon,game_title,greatest(0,coalesce(reward_points,0)),now() from eligible on conflict(user_id,achievement_id) do nothing returning reward_points) select count(*)::int,coalesce(sum(reward_points),0)::int into v_awards,v_points from inserted;
...
create or replace function public.record_game_metric(p_game_title text,p_metric text,p_value integer default 1,p_mode text default 'max') returns jsonb language plpgsql security definer set search_path=public as $$
declare v_uid uuid:=auth.uid(); v_game text:=trim(coalesce(p_game_title,'')); v_metric text:=lower(trim(coalesce(p_metric,''))); v_mode text:=lower(trim(coalesce(p_mode,'max'))); v_value int:=greatest(0,coalesce(p_value,0)); v_awards int; v_pending int; v_total int; v_add int;
begin
 if v_uid is null then raise exception 'Not signed in.'; end if; if lower(v_game) in ('slope','slope game') then v_game:='Slope'; end if; if lower(v_game) in ('a small world cup','a-small-world-cup','small world cup') then v_game:='A Small World Cup'; end if; if v_metric='score' then v_metric='best_score'; end if;
 insert into public.game_stats(user_id,game_title) values(v_uid,v_game) on conflict(user_id,game_title) do nothing;
 if v_game='A Small World Cup' and v_metric='goals' and v_mode='add' then select pending_signals into v_pending from public.aswc_goal_signal_pairs where user_id=v_uid for update; v_pending:=coalesce(v_pending,0); v_total:=v_pending+greatest(v_value,1); v_add:=floor(v_total::numeric/2)::int; v_pending:=mod(v_total,2); insert into public.aswc_goal_signal_pairs(user_id,pending_signals,updated_at) values(v_uid,v_pending,now()) on conflict(user_id) do update set pending_signals=excluded.pending_signals,updated_at=now(); update public.game_stats set goals=coalesce(goals,0)+v_add,last_played_at=now() where user_id=v_uid and game_title=v_game;
 elsif v_metric='best_score' then update public.game_stats set best_score=case when v_mode='replace' then v_value when v_mode='add' then coalesce(best_score,0)+greatest(v_value,1) else greatest(coalesce(best_score,0),v_value) end,last_played_at=now() where user_id=v_uid and game_title=v_game;
 elsif v_metric='goals' then update public.game_stats set goals=case when v_mode='replace' then v_value when v_mode='add' then coalesce(goals,0)+greatest(v_value,1) else greatest(coalesce(goals,0),v_value) end,last_played_at=now() where user_id=v_uid and game_title=v_game;
 elsif v_metric='wins' then update public.game_stats set wins=case when v_mode='replace' then v_value when v_mode='add' then coalesce(wins,0)+greatest(v_value,1) else greatest(coalesce(wins,0),v_value) end,last_played_at=now() where user_id=v_uid and game_title=v_game;
 elsif v_metric='fish_caught' then update public.game_stats set fish_caught=case when v_mode='replace' then v_value when v_mode='add' then coalesce(fish_caught,0)+greatest(v_value,1) else greatest(coalesce(fish_caught,0),v_value) end,last_played_at=now() where user_id=v_uid and game_title=v_game;
 elsif v_metric='money_earned' then update public.game_stats set money_earned=case when v_mode='replace' then v_value when v_mode='add' then coalesce(money_earned,0)+greatest(v_value,1) else greatest(coalesce(money_earned,0),v_value) end,last_played_at=now() where user_id=v_uid and game_title=v_game;
 elsif v_metric='checks' then update public.game_stats set checks=case when v_mode='replace' then v_value when v_mode='add' then coalesce(checks,0)+greatest(v_value,1) else greatest(coalesce(checks,0),v_value) end,last_played_at=now() where user_id=v_uid and game_title=v_game;
 elsif v_metric='checkmates' then update public.game_stats set checkmates=case when v_mode='replace' then v_value when v_mode='add' then coalesce(checkmates,0)+greatest(v_value,1) else greatest(coalesce(checkmates,0),v_value) end,last_played_at=now() where user_id=v_uid and game_title=v_game;
 elsif v_metric='merges' then update public.game_stats set merges=case when v_mode='replace' then v_value when v_mode='add' then coalesce(merges,0)+greatest(v_value,1) else greatest(coalesce(merges,0),v_value) end,last_played_at=now() where user_id=v_uid and game_title=v_game;
 elsif v_metric='best_level' then update public.game_stats set best_level=case when v_mode='replace' then v_value when v_mode='add' then coalesce(best_level,0)+greatest(v_value,1) else greatest(coalesce(best_level,0),v_value) end,last_played_at=now() where user_id=v_uid and game_title=v_game;
 elsif v_metric='hits' then update public.game_stats set hits=case when v_mode='replace' then v_value when v_mode='add' then coalesce(hits,0)+greatest(v_value,1) else greatest(coalesce(hits,0),v_value) end,last_played_at=now() where user_id=v_uid and game_title=v_game;
 else update public.game_stats set play_count=case when v_metric='play_count' then coalesce(play_count,0)+1 else play_count end,last_played_at=now() where user_id=v_uid and game_title=v_game; end if;
 v_awards:=public.award_ready_achievements(v_uid,v_game); return (select to_jsonb(gs)||jsonb_build_object('new_awards',v_awards) from public.game_stats gs where gs.user_id=v_uid and gs.game_title=v_game);
end $$;
grant execute on function public.record_game_metric(text,text,integer,text) to authenticated;

create or replace function public.record_slope_score(p_score integer) returns jsonb language plpgsql security definer set search_path=public as $$ begin return public.record_game_metric('Slope','best_score',p_score,'max'); end $$; grant execute on function public.record_slope_score(integer) to authenticated;
create or replace function public.report_slope_score(p_score integer) returns jsonb language plpgsql security definer set search_path=public as $$ begin return public.record_slope_score(p_score); end $$; grant execute on function public.report_slope_score(integer) to authenticated;
create or replace function public.submit_slope_score(p_score integer) returns jsonb language plpgsql security definer set search_path=public as $$ begin return public.record_slope_score(p_score); end $$; grant execute on function public.submit_slope_score(integer) to authenticated;
create or replace function public.record_aswc_goal_delta_v93(p_delta integer default 1) returns jsonb language plpgsql security definer set search_path=public as $$ begin return public.record_game_metric('A Small World Cup','goals',greatest(1,coalesce(p_delta,1)),'add'); end $$; grant execute on function public.record_aswc_goal_delta_v93(integer) to authenticated;
create or replace function public.record_aswc_win_v93() returns jsonb language plpgsql security definer set search_path=public as $$ begin return public.record_game_metric('A Small World Cup','wins',1,'add'); end $$; grant execute on function public.record_aswc_win_v93() to authenticated;

-- v115 fixed: drop old leaderboard function before recreating it with a new return type.
drop function if exists public.get_public_leaderboard(integer);
drop function if exists public.get_public_leaderboard();

create or replace function public.get_public_leaderboard(max_rows integer default 100) returns table(id uuid,username text,avatar_url text,points integer,equipped_icon text,rank text,true_member boolean,last_seen_at timestamptz,equipped_banner_id text,equipped_banner_name text,equipped_banner_gradient text) language sql security definer set search_path=public as $$ select p.id,p.username,p.avatar_url,p.points,p.equipped_icon,p.rank,p.true_member,p.last_seen_at,p.equipped_banner_id,p.equipped_banner_name,p.equipped_banner_gradient from public.profiles p order by p.points desc, lower(p.username) asc limit greatest(1,least(coalesce(max_rows,100),100)); $$;
grant execute on function public.get_public_leaderboard(integer) to anon, authenticated;

-- Backfill achievements from existing stats
select set_config('blank_space.allow_points_update','1',true);
with eligible as (select gs.user_id,c.* from public.game_stats gs join public.achievement_catalog c on c.game_title=gs.game_title left join public.user_achievements ua on ua.user_id=gs.user_id and ua.achievement_id=c.achievement_id where ua.achievement_id is null and (case c.stat when 'play_count' then coalesce(gs.play_count,0) when 'best_score' then coalesce(gs.best_score,0) when 'goals' then coalesce(gs.goals,0) when 'wins' then coalesce(gs.wins,0) when 'fish_caught' then coalesce(gs.fish_caught,0) when 'money_earned' then coalesce(gs.money_earned,0) when 'checks' then coalesce(gs.checks,0) when 'checkmates' then coalesce(gs.checkmates,0) when 'merges' then coalesce(gs.merges,0) when 'best_level' then coalesce(gs.best_level,0) when 'hits' then coalesce(gs.hits,0) else 0 end)>=coalesce(c.target,999999999)), inserted as (insert into public.user_achievements(user_id,achievement_id,title,description,icon,game_title,reward_points,earned_at) select user_id,achievement_id,title,description,icon,game_title,greatest(0,coalesce(reward_points,0)),now() from eligible on conflict(user_id,achievement_id) do nothing returning user_id,reward_points), per_user as (select user_id,coalesce(sum(reward_points),0)::int pts from inserted group by user_id) update public.profiles p set points=coalesce(p.points,0)+per_user.pts from per_user where p.id=per_user.user_id and per_user.pts>0;

create or replace function public.blank_space_v115_healthcheck() returns table(check_name text,result text) language sql security definer set search_path=public as $$ select 'achievement_catalog_rows',count(*)::text from public.achievement_catalog union all select 'slope_rows',count(*)::text from public.achievement_catalog where game_title='Slope' union all select 'true_member_column',case when exists(select 1 from information_schema.columns where table_schema='public' and table_name='profiles' and column_name='true_member') then 'yes' else 'no' end union all select 'banners_table',case when to_regclass('public.user_banners') is not null then 'yes' else 'no' end union all select 'friend_blocks_table',case when to_regclass('public.friend_blocks') is not null then 'yes' else 'no' end union all select 'game_presence_table',case when to_regclass('public.game_presence') is not null then 'yes' else 'no' end union all select 'status','OK - Blank Space v116 update installed'; $$;
grant execute on function public.blank_space_v115_healthcheck() to anon, authenticated;
select pg_notify('pgrst','reload schema');
select * from public.blank_space_v115_healthcheck();




-- ============================================================
-- Blank Space v116 repair layer
-- Fixes presence, True Member, banners, timed messages, and achievements.
-- ============================================================
...
alter table public.admin_user_messages add column if not exists active boolean default true;
alter table public.admin_user_messages add column if not exists created_at timestamptz default now();
alter table public.admin_user_messages add column if not exists expires_at timestamptz default (now() + interval '20 seconds');
...
create or replace function public.is_blank_space_owner_or_admin_v116()
returns boolean
language sql
stable
security definer
...
$$;
grant execute on function public.is_blank_space_owner_or_admin_v116() to anon, authenticated;
...
set search_path = public
as $$
  select public.is_blank_space_owner_or_admin_v116();
$$;
grant execute on function public.is_blank_space_owner_or_admin_v115() to anon, authenticated;

create or replace function public.heartbeat_presence_v116(p_device_id text default null)
returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  ...
  set last_seen_at = now(),
      active_device_id = left(coalesce(p_device_id,''), 120)
  where id = auth.uid();

  return jsonb_build_object('ok', true, 'last_seen_at', now());
end $$;
grant execute on function public.heartbeat_presence_v116(text) to authenticated;

drop function if exists public.admin_set_true_member_v115(text, boolean);
create function public.admin_set_true_member_v115(p_username text, p_true_member boolean default true)
returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  if auth.uid() is null or not public.is_blank_space_owner_or_admin_v116() then
  ...
  update public.profiles
  set true_member = coalesce(p_true_member, true)
  where id = v_target.id;

  insert into public.admin_actions_log(admin_id,target_user_id,target_username,action,reason)
  values(auth.uid(), v_target.id, v_target.username, case when p_true_member then 'make_true_member' else 'remove_true_member' end, null);

  return jsonb_build_object('ok', true, 'username', v_target.username, 'true_member', coalesce(p_true_member,true));
end $$;
grant execute on function public.admin_set_true_member_v115(text, boolean) to authenticated;

drop function if exists public.admin_send_user_message_v115(text, text, integer, text, boolean);
create function public.admin_send_user_message_v115(
  p_username text,
  p_message text,
...
declare
  v_target public.profiles%rowtype;
  v_count integer := 0;
  v_seconds integer := greatest(5, least(coalesce(p_seconds,20), 3600));
begin
  if auth.uid() is null or not public.is_blank_space_owner_or_admin_v116() then
    raise exception 'Owner/Admin only.';
  end if;
  ...
  if coalesce(p_send_all,false) then
    insert into public.admin_user_messages(target_user_id, sender_id, title, label, message, style, active, expires_at)
    select id, auth.uid(), 'Blank Space Notice', 'Blank Space Message', left(p_message,500), coalesce(p_style,'notice'), true, now() + (v_seconds || ' seconds')::interval
    from public.profiles
    where id <> auth.uid();
    ...
    insert into public.admin_user_messages(target_user_id, sender_id, title, label, message, style, active, expires_at)
    values(v_target.id, auth.uid(), 'Blank Space Notice', 'Blank Space Message', left(p_message,500), coalesce(p_style,'notice'), true, now() + (v_seconds || ' seconds')::interval);
...
-- Slope targets made easier and cleaned.
update public.achievement_catalog
set target = case
  when achievement_id like 'slope_%' and target >= 250 then 175
  when achievement_id like 'slope_%' and target >= 200 then 150
  when achievement_id like 'slope_%' and target >= 175 then 130
  when achievement_id like 'slope_%' and target >= 150 then 110
  when achievement_id like 'slope_%' and target >= 130 then 90
  else target
end,
stat = 'best_score',
game_title = 'Slope'
...
begin
  if v_auth is not null and p_user_id is distinct from v_auth and not public.is_blank_space_owner_or_admin_v116() then
...
begin
  ...
  return jsonb_build_object('ok', true, 'game_title', v_game, 'new_awards', v_awards);
end $$;
...
begin
  ...
  if v_game = 'A Small World Cup' and v_metric = 'goals' and v_mode = 'add' then
  ...
  return (
    select to_jsonb(gs) || jsonb_build_object('new_awards', v_awards)
...
-- Backfill missing achievements from existing stats without duplicating points.
do $$
declare
  r record;
  v_added integer := 0;
begin
  for r in
    select distinct user_id, game_title from public.game_stats
  loop
    v_added := public.award_ready_achievements(r.user_id, r.game_title);
  end loop;
end $$;

drop function if exists public.blank_space_v116_healthcheck();
create function public.blank_space_v116_healthcheck()
returns table(check_name text, result text)
language sql
security definer
set search_path = public
as $$
  select 'true_member_column', case when exists(select 1 from information_schema.columns where table_schema='public' and table_name='profiles' and column_name='true_member') then 'yes' else 'no' end
  union all select 'sender_id_column', case when exists(select 1 from information_schema.columns where table_schema='public' and table_name='admin_user_messages' and column_name='sender_id') then 'yes' else 'no' end
  union all select 'banner_table', case when to_regclass('public.user_banners') is not null then 'yes' else 'no' end
  union all select 'presence_function', case when to_regprocedure('public.heartbeat_presence_v116(text)') is not null then 'yes' else 'no' end
  union all select 'leaderboard_function', case when to_regprocedure('public.get_public_leaderboard(integer)') is not null then 'yes' else 'no' end
  ...
  union all select 'status', 'OK - Blank Space v116 repair update installed';
$$;
grant execute on function public.blank_space_v116_healthcheck() to anon, authenticated;

select pg_notify('pgrst','reload schema');

select * from public.blank_space_v116_healthcheck();
