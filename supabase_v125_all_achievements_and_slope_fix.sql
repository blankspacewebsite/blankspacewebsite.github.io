-- Blank Space v125: all achievements + strict Slope false-15 fix
-- Paste this whole file into Supabase SQL Editor and click Run.
--
-- This re-syncs all 240 website achievements, restores all generic metric functions,
-- rejects false Slope score 15 at the database level, and corrects Owner's current Slope false-15 case
-- to the real score 6.
...
begin
  if exists (select 1 from public.game_stats group by user_id, game_title having count(*) > 1) then
    create temp table tmp_v125_game_stats on commit drop as
...
where a.achievement_id = d.achievement_id and a.ctid <> d.keep_ctid;
...
begin
  if exists (select 1 from public.user_achievements group by user_id, achievement_id having count(*) > 1) then
    create temp table tmp_v125_user_ach on commit drop as
...
('slope_best_score_75_20','Get 16 points in Slope','Get 16 points in one Slope round','🟢','Slope','best_score',16,2,'easy'),
...
begin
  ...
  if v_auth is not null and p_user_id is distinct from v_auth and not exists (
    select 1 from public.profiles p where p.id = v_auth and lower(trim(coalesce(p.rank,'member'))) in ('owner','admin')
  ) then
...
begin
  ...
  return (select to_jsonb(gs) || jsonb_build_object('ok', true, 'new_awards', v_awards) from public.game_stats gs where gs.user_id = v_uid and gs.game_title = v_game);
end;
$$;
...
begin
  ...
  if v_game = 'Slope' and v_metric = 'best_score' and v_value = 15 then
    return jsonb_build_object('ok', false, 'ignored', true, 'reason', 'Rejected false Slope score 15');
  end if;
  ...
  if v_game = 'A Small World Cup' and v_metric = 'goals' and v_mode in ('add','delta_v93','add_goal_v93','goal_delta') then
    ...
    update public.game_stats set goals = coalesce(goals,0) + v_add, last_played_at = now()
    ...
    update public.game_stats set wins = coalesce(wins,0) + greatest(v_value,1), last_played_at = now()
    where user_id = v_uid and game_title = v_game;
  else
    execute format(
...
-- Correct Owner's current false Slope 15 to the real score 6.
do $$
declare
  v_user_id uuid;
...
begin
  ...
  if v_user_id is not null then
    ...
    perform set_config('blank_space.allow_points_update','1',true);
    update public.profiles set points = greatest(0, coalesce(points,0) - v_points_to_remove)
    where id = v_user_id;

    insert into public.game_stats(user_id, game_title, best_score, play_count, first_played_at, last_played_at)
    values(v_user_id, 'Slope', 6, 1, now(), now())
    on conflict(user_id, game_title) do update set
      best_score = 6,
      last_played_at = now();

    v_awards := public.award_ready_achievements(v_user_id, 'Slope');

    raise notice 'Corrected Owner Slope false 15. Removed % Slope achievements, subtracted % wrong points, set score 6, awarded % correct Slope achievements.',
      v_removed, v_points_to_remove, v_awards;
  end if;
end $$;

drop function if exists public.backfill_all_achievements_v125();

create function public.backfill_all_achievements_v125()
returns jsonb
language plpgsql
security definer
set search_path = public
...
select public.backfill_all_achievements_v125() as backfill_result;

drop function if exists public.blank_space_v125_achievements_healthcheck();

create function public.blank_space_v125_achievements_healthcheck()
...
as $$
  ...
  union all select 'slope_target_15_rows', count(*)::text from public.achievement_catalog where game_title='Slope' and target=15
  union all select 'owner_slope_best_score', coalesce((select gs.best_score::text from public.profiles p join public.game_stats gs on gs.user_id=p.id and gs.game_title='Slope' where lower(trim(p.username))=lower(trim('Owner')) order by p.created_at asc nulls last, p.id asc limit 1),'none')
  union all select 'status', 'OK - v125 all achievements and strict Slope fixed';
$$;

grant execute on function public.blank_space_v125_achievements_healthcheck() to anon, authenticated;

select pg_notify('pgrst','reload schema');

select * from public.blank_space_v125_achievements_healthcheck();
