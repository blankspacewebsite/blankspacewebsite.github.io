-- Blank Space v91 Supabase update
-- Run this BEFORE uploading the v91 GitHub files.
--
-- This fixes achievements and cleans up the bad Slope fallback data.
-- It restores Slope achievements to real score-based goals.
-- It does NOT delete accounts, friends, chats, icons, ranks, suggestions, or non-Slope achievements.
-- It DOES remove Slope achievement rows and subtract their Slope reward points so they can be re-earned from real Slope scores.
...
update public.game_stats set
  play_count=coalesce(play_count,0), play_seconds=coalesce(play_seconds,0),
  first_played_at=coalesce(first_played_at,now()), last_played_at=coalesce(last_played_at,now()),
  best_score=coalesce(best_score,0), goals=coalesce(goals,0), wins=coalesce(wins,0),
  fish_caught=coalesce(fish_caught,0), money_earned=coalesce(money_earned,0),
  checks=coalesce(checks,0), checkmates=coalesce(checkmates,0), merges=coalesce(merges,0),
  best_level=coalesce(best_level,0), hits=coalesce(hits,0);
...
update public.user_achievements set
  title=coalesce(nullif(title,''),'Achievement'), description=coalesce(description,''),
  icon=coalesce(nullif(icon,''),'🏆'), reward_points=coalesce(reward_points,0), earned_at=coalesce(earned_at,now());
...
-- =========================
-- 2) De-duplicate rows so ON CONFLICT works
-- =========================

do $$
begin
  if exists (select 1 from public.game_stats group by user_id, game_title having count(*) > 1) then
    create temp table tmp_v91_game_stats_merged on commit drop as
    ...
    select user_id, game_title, play_count, play_seconds, first_played_at, last_played_at, best_score, goals, wins, fish_caught, money_earned, checks, checkmates, merges, best_level, hits from tmp_v91_game_stats_merged;
  end if;
end $$;
...
begin
  if exists (select 1 from public.user_achievements group by user_id, achievement_id having count(*) > 1) then
    create temp table tmp_v91_user_achievements_merged on commit drop as
    ...
    from public.user_achievements order by user_id, achievement_id, earned_at asc;
    ...
    select user_id, achievement_id, title, description, icon, game_title, reward_points, earned_at from tmp_v91_user_achievements_merged;
  end if;
end $$;
...
begin
  if exists (select 1 from public.achievement_catalog where achievement_id is not null group by achievement_id having count(*) > 1) then
    create temp table tmp_v91_catalog_merged on commit drop as
    select distinct on (achievement_id) achievement_id, title, description, icon, game_title, stat, target, reward_points, difficulty
    from public.achievement_catalog where achievement_id is not null order by achievement_id;
    ...
    select achievement_id, title, description, icon, game_title, stat, target, reward_points, difficulty from tmp_v91_catalog_merged;
  end if;
end $$;
...
-- =========================
-- 3) Remove bad Slope fallback and clean Slope achievement/progress only
-- =========================
...
declare
  r record;
begin
  -- Remove all Slope achievement rows and subtract only their reward points.
  -- This clears the accidental fake/play-count Slope awards so real Slope scores can earn them correctly.
  for r in
    with deleted as (
    ...
    from deleted group by user_id
  loop
    if r.points_to_remove > 0 then
      perform set_config('blank_space.allow_points_update','1',true);
      update public.profiles set points = greatest(0, coalesce(points,0) - r.points_to_remove) where id = r.user_id;
    end if;
  end loop;

  -- Remove only the saved Slope score because older fake/fallback attempts may have written fake best_score.
  update public.game_stats
  set best_score = 0, last_played_at = now()
  where lower(trim(game_title)) in ('slope','slope game');
end $$;

-- =========================
-- 4) Full achievement catalog from the v91 website
-- =========================
...
on conflict (achievement_id) do update set
  title=excluded.title, description=excluded.description, icon=excluded.icon, game_title=excluded.game_title,
  stat=excluded.stat, target=excluded.target, reward_points=excluded.reward_points, difficulty=excluded.difficulty;
...
-- =========================
-- 6) Achievement functions: save metric and award ALL matching achievements
-- =========================
...
begin
  ...
  with eligible as (
    ...
    select p_user_id, c.achievement_id, coalesce(nullif(c.title,''),'Achievement'), coalesce(c.description,''), coalesce(nullif(c.icon,''),'🏆'), c.game_title, greatest(0,coalesce(c.reward_points,0))
    from eligible c
...
begin
  ...
  -- This may award play_count achievements for games that intentionally use play_count.
  v_new_awards := public.award_ready_achievements(v_uid, v_game_title);

  select to_jsonb(gs) || jsonb_build_object('new_awards', v_new_awards) into v_stat
  from public.game_stats gs where gs.user_id = v_uid and gs.game_title = v_game_title;
...
begin
  ...
  if v_metric = 'money_earned' then v_value := least(v_value, 100000000);
  elsif v_metric in ('best_score','fish_caught','merges','hits') then v_value := least(v_value, 1000000);
  else v_value := least(v_value, 100000); end if;
...
begin
  if v_uid is null then raise exception 'Not signed in.'; end if;
  if trim(coalesce(p_achievement_id,'')) = '' then raise exception 'Achievement id is required.'; end if;
...
-- =========================
-- 7) Backfill missing non-Slope achievements from existing stats
-- =========================

drop function if exists public.force_award_missing_non_slope_achievements_for_all_users();
create function public.force_award_missing_non_slope_achievements_for_all_users()
returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  with eligible as (
    select gs.user_id, c.achievement_id, coalesce(nullif(c.title,''),'Achievement') as title, coalesce(c.description,'') as description, coalesce(nullif(c.icon,''),'🏆') as icon, c.game_title, greatest(0,coalesce(c.reward_points,0)) as reward_points
    from public.achievement_catalog c
    join public.game_stats gs on gs.game_title = c.game_title
    left join public.user_achievements ua on ua.user_id = gs.user_id and ua.achievement_id = c.achievement_id
    where ua.achievement_id is null
      and c.achievement_id is not null
      and c.game_title is not null
    ...
    insert into public.user_achievements (user_id, achievement_id, title, description, icon, game_title, reward_points)
    select user_id, achievement_id, title, description, icon, game_title, reward_points from eligible
    on conflict (user_id, achievement_id) do nothing
    returning user_id, reward_points
  ), per_user as (
    select user_id, count(*)::integer awards, coalesce(sum(reward_points),0)::integer points from inserted group by user_id
  ), updated as (
    update public.profiles p set points = coalesce(p.points,0) + per_user.points from per_user where p.id = per_user.user_id
    returning per_user.user_id, per_user.awards, per_user.points
  )
  select coalesce(sum(awards),0)::integer, coalesce(sum(points),0)::integer, count(*)::integer into v_awards, v_points, v_users from updated;
  return jsonb_build_object('non_slope_achievements_added',coalesce(v_awards,0),'points_added',coalesce(v_points,0),'users_updated',coalesce(v_users,0));
end;
$$;

revoke execute on function public.force_award_missing_non_slope_achievements_for_all_users() from public, anon, authenticated;
select set_config('blank_space.allow_points_update','1',true);
select public.force_award_missing_non_slope_achievements_for_all_users() as non_slope_backfill_result;

-- =========================
-- 8) Health check
-- =========================

drop function if exists public.blank_space_v91_achievement_healthcheck();
create function public.blank_space_v91_achievement_healthcheck()
...
as $$
  ...
  union all select 'slope_catalog_score_rows', count(*)::text from public.achievement_catalog where game_title = 'Slope' and stat = 'best_score'
  union all select 'slope_catalog_play_count_rows_should_be_0', count(*)::text from public.achievement_catalog where game_title = 'Slope' and stat = 'play_count'
  union all select 'slope_achievement_rows_after_cleanup_should_be_0', count(*)::text from public.user_achievements where achievement_id in ('slope_best_score_25_18', 'slope_best_score_50_19', 'slope_best_score_75_20', 'slope_best_score_100_21', 'slope_best_score_125_22', 'slope_best_score_150_23', 'slope_best_score_200_24', 'slope_best_score_250_25', 'slope_best_score_300_26', 'slope_best_score_400_27', 'slope_best_score_500_28', 'slope_best_score_650_29', 'slope_best_score_800_30', 'slope_best_score_1000_31', 'slope_best_score_1250_32', 'slope_best_score_1100_201', 'slope_best_score_1150_211', 'slope_best_score_1200_221', 'slope_best_score_1250_231', 'slope_best_score_1300_241')
  union all select 'record_game_metric_function', 'installed'
  union all select 'status', 'OK - run GitHub v91, then play real Slope to earn Slope achievements';
$$;

grant execute on function public.blank_space_v91_achievement_healthcheck() to anon, authenticated;
select pg_notify('pgrst','reload schema');
select * from public.blank_space_v91_achievement_healthcheck();
