-- Blank Space v143: hard fix custom Pause Website message
-- Run this in Supabase, then upload Blank Space v143.zip.
...
create or replace function public.set_website_pause_v143(
  p_paused boolean,
...
grant execute on function public.set_website_pause_v143(boolean,text[],text) to authenticated;

create or replace function public.get_website_pause_status_v143()
returns jsonb
language sql
security definer
set search_path = public
...
grant execute on function public.get_website_pause_status_v143() to anon, authenticated;
...
as $$
begin
  return public.set_website_pause_v143(p_paused, p_except_ranks, p_message);
end;
$$;
...
set search_path = public
as $$
  select public.get_website_pause_status_v143();
$$;
...
select 'OK - v143 custom Pause Website message installed' as status;
